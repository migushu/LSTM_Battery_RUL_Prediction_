...
class lstm_network():
    ...
    def build_network(self, time_step, features_num =1, dropout_prob=0.2,dense_units = 1,lstm_units = 50):
        # input_dim是输入的train_x的最后一个维度，train_x的维度为(n_samples, time_steps, input_dim)
        model = Sequential()
        #每个时间步经过第一个LSTM后，得到的中间隐向量是50维，一个20个时间步的样本数据进去得到（20×50）的数据
        model.add(LSTM(units=lstm_units, return_sequences=True, input_shape=(time_step, features_num)))
        print(model.layers)
